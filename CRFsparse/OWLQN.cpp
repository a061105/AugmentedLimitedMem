...
#include "TerminationCriterion.cpp"
#include <ctime>
#include <vector>
#include <deque>
#include <cmath>
...
void OWLQN::minimize(Problem* function) {
    for (int i=0;i<function->d;i++)
        function->wholeset.push_back(i);
    
    initial.clear();
    for (int i=0;i<function->d;i++)
        initial.push_back(function->w[i]);
    cerr<<"time(ms) Relative-function-value-difference"<<endl;
    clock_t start = clock();    
	
    OptimizerState state(function, initial, m,lambda, quiet);
    quiet=true;
	if (!quiet) {
		cerr << setprecision(15) << scientific << right;
		cerr << endl << "Optimizing function of " << state.dim << " variables with OWL-QN parameters:" << endl;
		cerr << "   l1 regularization weight: " << lambda << "." << endl;
		cerr << "   L-BFGS memory parameter (m): " << m << endl;
		cerr << "   Convergence tolerance: " << epsilon << endl;
		cerr << endl;
    ...
    clock_t iniend = clock();
    double t =(double)(iniend-start)/CLOCKS_PER_SEC *1000.0;  
    cerr<<t<<" ";
    cerr<<setprecision(20)<<state.value<<endl;
    cout<<setprecision(15)<<t<<" 0"<<endl; 
    while (true) {
		state.UpdateDir();
		...
		if (!quiet) {
			cerr << "Iter " << setw(4) << state.iter << ":  " << setw(20) << state.value;
			cerr << str.str() << flush;
		}

		if (termCritVal < epsilon){
            //cerr << "termination criterion attained."<<endl; 
            break;
        }

		state.Shift();
        clock_t end = clock();
        cerr<<(double)(end-start)/CLOCKS_PER_SEC *1000.0<<" ";
        cerr<<setprecision(20)<<state.value<<endl;
        int nnw=0;
        for (int i=0;i<state.x.size();i++)
            if(fabs(state.x[i]) > 1e-12)
                nnw++;
        cout<<setprecision(15)<<(double)(end-start)/CLOCKS_PER_SEC *1000.0<<" "<<nnw<<endl; 
