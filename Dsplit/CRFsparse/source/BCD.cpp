...
void bCD::minimize(Problem* prob){
	...
	for(i = 0,j = 0;i < numBlock;i++) {
		if(i != numBlock - 1) {
			range.push_back(make_pair(j*prob->K, (j + (prob->raw_d+1)/numBlock)*prob->K));
			cerr << j*prob->K << "," << (j + (prob->raw_d+1)/numBlock)*prob->K << endl;
			j += (prob->raw_d+1)/numBlock;
		}
		else {
			range.push_back(make_pair(j*prob->K, prob->d));
			cerr << j*prob->K << "," << prob->d << endl;
		}
	}
	//for(i=0;i<db;i++)
	//	w[i] = 0.0; 	
	...
	for (i=0;i<numBlock;i++){
		for(j=range[i].first;j<range[i].second;j++)
			w[j-range[i].first] = 0;
		swapper->save(i, w, range[i].second-range[i].first, "w");
	}
	...
	double normsg0 = 0; //l1 norm of the subgradients for the initial variables, 
	...
	//***************** outer loop starts ********************
	cerr << "outer loop" << endl;

	for (Int iter = 0; iter<max_iter; iter++){
		for(Int block = 0;block < numBlock;block++) {
			swapper->load(block, w, range[block].second-range[block].first, "w");
			for(Int QN_iter = 0; QN_iter < max_QN_iter;QN_iter++) {
				//cerr << "QN iter = " << QN_iter << ", block = " << block << ", iter = " << iter << endl;
				M = 0;
				if(QN_iter == 0) {
					epoch_iter = -1;
				}
				...
				if(QN_iter == 0) {
					act_set.clear();
					for(Int k = range[block].first;k < range[block].second;k++) {
						act_set.push_back(k - range[block].first);
					}
					Ddiag.clear(); L.clear(); Sps.clear();
					for (j=0;j<act_set.size();j++){
						Bdiag[j]=0.0;
						Bd[j]=0.0;
					}
					prob->grad(act_set,range[block],shg);
					...
					//l2_reg = theta * l2_norm_sq(w,act_set)/2.0;
					//curF = prob->fun() + nonsmooth + l2_reg;
					curF = prob->fun(act_set) + nonsmooth;
				}
				//cerr << "curF = " << curF << endl;
				...
				innerAct = act_set; 
				i_iter = (Int)(range[block].second-range[block].first)/act_set.size();
				if (i_iter>max_inner)
					i_iter = max_inner;
				...
				//cerr << "before compute_fv_change" << endl;
				prob->compute_fv_change(w_change,act_set, range[block], fv_change);
				//cerr << "after compute_fv_change" << endl;

				//cerr << "before update_fvalue" << endl;
				prob->update_fvalue(fv_change,alpha);
				//cerr << "after update_fvalue" << endl;
				...
				candF = prob->fun(act_set) + nonsmooth ;
				//cerr << "fun = " << prob->fun(act_set) << ", nonsmooth = " << nonsmooth << endl;
			...
			cout << "act_set size = " << act_set.size() << endl;
			cerr << "act_set size = " << act_set.size() << endl;
			swapper->save(block, w, range[block].second-range[block].first, "w");
		}
		nonsmooth = 0.0;
		for(Int block=0;block<numBlock;block++) {
			swapper->load(block, w, range[block].second-range[block].first, "w");
		...
		cout << "fun value for all block = " << prob->fun(numBlock, range, swapper) + nonsmooth << endl;
		cerr << "fun value for all block = " << prob->fun(numBlock, range, swapper) + nonsmooth << endl;
		...
		cout <<setw(6) << iter+1 <<setw(20)<< (double)(cur_time - start_time)/CLOCKS_PER_SEC * 1.0 <<setw(20)<<curF<<setw(12)<<act_set.size() << setw(20)<<prob->train_accuracy()<< setw(20) << nonsmooth << endl;
